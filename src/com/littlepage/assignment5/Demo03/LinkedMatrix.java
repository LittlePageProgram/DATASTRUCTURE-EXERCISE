...
public class LinkedMatrix  {
    ...
    public void transposition(){
        ...
        while(it.hasNext()){
            Data temp=it.next();
            int tempInt=temp.getRow();
            temp.setRow(temp.getColumn());
            temp.setColumn(tempInt);
        }
    }

    /**
     * plus the linkedMatrix to this matrix
     * @param linkedMatrix
     */
    public void plus(LinkedMatrix linkedMatrix){
        Iterator<Data> it=linkedMatrix.matrix.iterator();
        while(it.hasNext()){
           Data temp= it.next();
           plus(temp);
        }
    ...
    /**
     * for the plus linkedMatrix method
     * @param data
     * @return true if plus truly,false if plus
     */
    private void plus(Data data){
        if(data.getRow()>=row||data.getColumn()>=column) return ;
        Iterator<Data> it=matrix.iterator();
        while(it.hasNext()){
            Data temp=it.next();
            if(temp.getRow()==data.getRow()&&temp.getColumn()==data.getColumn()){
                temp.setData(data.getData()+temp.getData());
                return ;
            }
        }
        matrix.add(data);
